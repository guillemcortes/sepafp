...
class UNetP(BaseModel):
    def __init__(self, sample_rate, fft_size, hop_size, window_size, kernel_size, stride, device='cuda', eps=1e-8, mask_logit=False):
        ...
        #filterbank
        self.enc_fb = asteroid_filterbanks.ParamSincFB(513, self.fft_size+5, stride=None, sample_rate=self.sample_rate, min_low_hz=50, min_band_hz=50)
        self.dec_fb = asteroid_filterbanks.ParamSincFB(513, self.fft_size+5, stride=None, sample_rate=self.sample_rate, min_low_hz=50, min_band_hz=50)
        self.enc = asteroid_filterbanks.enc_dec.Encoder(self.enc_fb)
        self.dec = asteroid_filterbanks.enc_dec.Decoder(self.dec_fb)
    ...
    def forward(self, x_in):
        ...
        X_in_speech = self.enc(x_in)


        # add channels dimension
        X = X_in.unsqueeze(1)
        X_in_speech = X_in_speech.unsqueeze(1)

        # input layer
        X_speech = self.input_layer_speech(X_in_speech)
        ...
        if self.mask_logit:
            speech = X_in_speech * torch.sigmoid(X_speech)
