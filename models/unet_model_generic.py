...
class UNet(BaseModel):
    def __init__(self, sample_rate, fft_size, hop_size, window_size, kernel_size, stride, sources, min_K=16, device='cuda', eps=1e-8, mask_logit=False):
        ...
        for i, src in enumerate(self.sources):
            ...
            linput_layer[src] = input_layer(1)

            ldown[src] = nn.ModuleList()
            ldown[src].append(down(1, self.K, self.kernel_size, self.stride,(1,1)))
            # down blocks
            for j in range(5):
                #print(self.K*(2**j), self.K*(2**(j+1)))
                ldown[src].append(down(self.K*(2**j), self.K*(2**(j+1)), self.kernel_size, self.stride,(2,2)))

            # down_speech_1 = down(1, 16, self.kernel_size, self.stride,(1,1))
            # down_speech_2 = down(16, 32, self.kernel_size, self.stride,(2,2))
            # down_speech_3 = down(32, 64, self.kernel_size, self.stride,(2,2))
            # down_speech_4 = down(64, 128, self.kernel_size, self.stride,(2,2))
            # down_speech_5 = down(128, 256, self.kernel_size, self.stride,(2,2))
            # down_speech_6 = down(256, 512, self.kernel_size, self.stride,(2,2))

            lup[src] = nn.ModuleList()
            lup[src].append(up(self.K*(2**5), self.K*(2**4), self.kernel_size, self.stride, (1,1),(2,2), 1))
            # up blocks
            for j in range(5,1,-1):
                #print(self.K*(2**j), self.K*(2**(j-2)))
                lup[src].append(up(self.K*(2**j), self.K*(2**(j-2)), self.kernel_size, self.stride, (1,1),(2,2), 7-j))
            # up_speech_1 = up(512, 256, self.kernel_size, self.stride, (1,1),(2,2), 1)
            # up_speech_2 = up(512, 128, self.kernel_size, self.stride, (1,1),(2,2), 2)
            # up_speech_3 = up(256, 64, self.kernel_size, self.stride, (1,1),(2,2), 3)
            # up_speech_4 = up(128, 32, self.kernel_size, self.stride, (1,1),(2,2), 4)
            # up_speech_5 = up(64, 16, self.kernel_size, self.stride, (1,1),(2,2), 5)

            # # up blocks for music
            # up_music_1 = up(512, 256, self.kernel_size, self.stride, (1,1),(2,2), 1)
            # up_music_2 = up(512, 128, self.kernel_size, self.stride, (1,1),(2,2), 2)
            # up_music_3 = up(256, 64, self.kernel_size, self.stride, (1,1),(2,2), 3)
            # up_music_4 = up(128, 32, self.kernel_size, self.stride, (1,1),(2,2), 4)
            # up_music_5 = up(64, 16, self.kernel_size, self.stride, (1,1),(2,2), 5)

            # last layer sigmoid
            llast_layer[src] = last_layer(self.K*2, 1, self.kernel_size, self.stride, (0, 1),(1,1))

        self.input_layer = nn.ModuleDict(linput_layer)
        self.up = nn.ModuleDict(lup)
        self.down = nn.ModuleDict(ldown)
        self.last_layer = nn.ModuleDict(llast_layer)
    ...
    def forward(self, x_in):
        ...
        X_ins = X_in.unsqueeze(1)
        X_est = []
        for i, src in enumerate(self.sources):
            # input layer
            X = self.input_layer[src](X_ins.clone())

            X_down = [self.down[src][0](X)]
            #down convolutions
            for j in range(1,6):
                X_down.append(self.down[src][j](X_down[j-1]))

            #up convolutions & concat
            X_src = X_down.pop()
            for j in range(5):
                X_u = self.up[src][j](X_src)
                X_d = X_down.pop()
                X_src = torch.cat([X_u, X_d], dim=1)

            X_est.append(self.last_layer[src](X_src).squeeze(1))

        X_s = torch.stack(X_est, dim=0)
        X_sum = X_s.sum(dim=0)

        if self.mask_logit:
            X_masked = torch.sigmoid(X_s)
        else:
            X_masked = torch.stack([X_in * X_est[i] / (X_sum + self.eps) for i in range(len(self.sources))])

        polar = torch.mul(X_masked,torch.cos(phase)) + torch.mul(X_masked,torch.sin(phase) * 1j)
        x_out = torch.stack([torch.istft(polar[i,...], self.fft_size, length=x_in.shape[1] ,hop_length=self.hop_size, window=window, return_complex=False, onesided=True, center=True, normalized=True) for i in range(len(self.sources))], dim=1)
        ...
        # for i, src in enumerate(self.sources):
        #     if self.mask_logit:
        #         X_masked = X_in * torch.sigmoid(X_est[i])
        #     else:
        #         # create the mask
        #         X_mask = X_est[i] / (X_sum + self.eps)
        #         # use mask to separate speech from mix
        #         X_masked = X_in * X_mask

        #     # istft
        #     polar = X_masked * torch.cos(phase) + X_masked * torch.sin(phase) * 1j
        #     x_out.append(torch.istft(polar, self.fft_size, length=x_in.shape[1] ,hop_length=self.hop_size, window=window, return_complex=False, onesided=True, center=True, normalized=True))
        #     x_out[i] = x_out[i].squeeze(1)

        #     # unnormalize
        #     x_out[i] = x_out[i] * std + mean

        # # add sources to a tensor to return them
        # return torch.stack([x_out[i] for i, src in enumerate(self.sources)], dim=1)
